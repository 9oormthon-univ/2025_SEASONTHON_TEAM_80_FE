...
const COMMON_STYLES = {
  ...
  stepBox: "absolute flex h-4 w-4 items-center justify-center rounded-md bg-red-100",
    ...
    <div className={`flex flex-col items-center ${className}`}>
      <div className={COMMON_STYLES.container}>
        ...
        <div className={`${COMMON_STYLES.stepBox} ${stepData.stepBoxPosition}`}>
          <span className={COMMON_STYLES.stepNumber}>
            {stepData.step}
          </span>
        </div>
        
        {stepData.lines.map((linePosition) => (
          <div key={linePosition} className={`${COMMON_STYLES.line} ${linePosition}`} />
        ))}
      </div>
    </div>
  );
